...
def experience_replay(env, episode_num, n_ep, n_exp):
  ...
  with tf.variable_scope("batch"):
    ...
    not_done = tf.gather_nd(nd, samples, name="not_done")
...
def model(env):
  ...
  n_ep = tf.placeholder(tf.int32,shape=[],name="n_ep")
  n_exp = tf.placeholder(tf.int32,shape=[],name="n_exp")
  ...
  nextQ = tf.reduce_sum(tf.multiply(ref("target/qvals:0"), policy_onehot), axis=[-1,-2], name="nextQ")
  ...
  predictedQ = tf.reduce_sum(tf.multiply(ref("main/qvals:0"), actions_onehot), axis=[-1,-2],
      name="predictedQ")
  ...
  masked_err = tf.multiply(inbounds_mask * latter_mask, td_err, name="masked_err")
  ...
  tf.summary.histogram("target_q", targetQ)
...
def train_model(sess, dbg, writer, save, save_best, env):
  ...
  try:
    while FLAGS.total_episodes is None or episode_num < FLAGS.total_episodes:
      ...
      for (t,s,a,r,_,s1,d) in epoch(dbg, env, "main/explore:0"):
        sess.run("add_experience", feed_dict={'a:0':a,'s:0':s,'r:0':r,'nd:0': not d})
        if episode_num >= FLAGS.buffer_size and (t % FLAGS.train_rate) == 0:
          if step % FLAGS.summary_rate == 0:
            _,smry = dbg.run(["train", "desc/desc:0"], fd)
            writer.add_summary(smry, global_step=step)
          else:
            dbg.run("train", feed_dict=fd)
